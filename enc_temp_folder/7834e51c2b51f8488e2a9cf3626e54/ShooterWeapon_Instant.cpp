...
AShooterWeapon_Instant::AShooterWeapon_Instant(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	CurrentAimUpValue = 0.0f;
	AimUpResetPeriod = 0.1f;
}
...
void AShooterWeapon_Instant::FireWeapon()
{
	...
	const float CurrentSpreadIncrement = GetCurrentSpreadIncrement();
	...
	// Shoot up
	FVector UpDir(0, 0, 1);
	ShootDir += UpDir * CurrentSpreadIncrement / 30;
...
void AShooterWeapon_Instant::AimUp()
{
	if (MyPawn && MyPawn->IsLocallyControlled()
		&& MyPawn->IsFirstPerson())
	{
		float aimUpIncrement = InstantConfig.FiringAimUpIncrement;
		if (CurrentAimUpValue == InstantConfig.FiringAimUpValueMax)
		{
			aimUpIncrement = 0.f;
		}
		...
		MyPawn->LookUpAtRate(-aimUpIncrement);
		CurrentAimUpValue = FMath::Min(InstantConfig.FiringAimUpValueMax, CurrentAimUpValue + aimUpIncrement);
	}
}

void AShooterWeapon_Instant::StopAimUp()
{
	// Reset recoil
	if (MyPawn && MyPawn->IsLocallyControlled())
	{
		if (CurrentAimUpValue > 0)
		{
			MyPawn->LookUpAtRate(CurrentAimUpValue);
			CurrentAimUpValue = 0.f;
		}
	}
}
...
void AShooterWeapon_Instant::OnBurstFinished()
{
	...
	if (MyPawn && MyPawn->IsLocallyControlled())
	{
		GetWorldTimerManager().SetTimer(
			AimUpResetTimer, 
			this, 
			&AShooterWeapon_Instant::StopAimUp, 
			AimUpResetPeriod);
	}
...
float AShooterWeapon_Instant::GetCurrentSpreadIncrement() const
{
