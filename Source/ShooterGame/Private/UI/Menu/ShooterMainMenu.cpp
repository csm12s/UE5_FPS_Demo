...
static const FString MapNames[] = { TEXT("Sanctuary"), TEXT("Highrise"), TEXT("IceWorld") };
static const FString JoinMapNames[] = { TEXT("Any"), TEXT("Sanctuary"), TEXT("Highrise"), TEXT("IceWorld") };
static const FName PackageNames[] = { TEXT("Sanctuary.umap"), TEXT("Highrise.umap"),TEXT("IceWorld.umap") };
...
void FShooterMainMenu::Construct(TWeakObjectPtr<UShooterGameInstance> _GameInstance, TWeakObjectPtr<ULocalPlayer> _PlayerOwner)
{
	...
	if (Identity.IsValid())
	{
		...
		{
			OnLoginCompleteConstructDelegateHandle = Identity->AddOnLoginCompleteDelegate_Handle(ControllerId, FOnLoginCompleteDelegate::CreateRaw(this, &FShooterMainMenu::OnLoginCompleteConstruct));
...
void FShooterMainMenu::OnQuickMatchSelectedLoginRequired()
{
	...
	if (Identity.IsValid())
	{
		...
		OnLoginCompleteQuickmatchDelegateHandle = Identity->AddOnLoginCompleteDelegate_Handle(ControllerId, FOnLoginCompleteDelegate::CreateRaw(this, &FShooterMainMenu::OnLoginCompleteQuickmatch));
...
void FShooterMainMenu::OnSplitScreenSelectedHostOnlineLoginRequired()
{
	...
	if (Identity.IsValid())
	{
		...
		{
			OnLoginCompleteHostOnlineDelegateHandle = Identity->AddOnLoginCompleteDelegate_Handle(ControllerId, FOnLoginCompleteDelegate::CreateRaw(this, &FShooterMainMenu::OnLoginCompleteHostOnline));
			Identity->Login(ControllerId, FOnlineAccountCredentials());
		}
	}
}
...
void FShooterMainMenu::OnLoginCompleteConstruct(int32 LocalUserNum, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error)
{
	IOnlineIdentityPtr Identity = Online::GetIdentityInterface(GetTickableGameObjectWorld());
	if (Identity.IsValid())
	{
...
void FShooterMainMenu::OnJoinServerLoginRequired()
{
	...
	if (Identity.IsValid())
	{
		...
		{
			OnLoginCompleteJoinDelegateHandle = Identity->AddOnLoginCompleteDelegate_Handle(ControllerId, FOnLoginCompleteDelegate::CreateRaw(this, &FShooterMainMenu::OnLoginCompleteJoin));
			Identity->Login(ControllerId, FOnlineAccountCredentials());
		}
	}
}
